...
public interface JudgeQuestionMapper {
  ...
  @Select(
      "select * from judge_question where questionId in (select questionId from paper_manage where"
          + " questionType = 3 and paperId = #{paperId})")
  ...
  @Select(
      "select questionId from judge_question  where subject=#{subject}  order by rand() desc limit"
          + " #{pageNo}")
