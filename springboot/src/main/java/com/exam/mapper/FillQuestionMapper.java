...
public interface FillQuestionMapper {
  ...
  @Select(
      "select * from fill_question where questionId in (select questionId from paper_manage where"
          + " questionType = 2 and paperId = #{paperId})")
  ...
  @Select(
      "select questionId from fill_question where subject = #{subject} order by rand() desc limit"
          + " #{pageNo}")
