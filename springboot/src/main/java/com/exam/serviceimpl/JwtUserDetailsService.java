...
public class JwtUserDetailsService implements UserDetailsService {
  ...
  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    ...
    if (student != null) {
      return new User(username, student.getPwd(), new ArrayList<>());
    }
    Teacher teacher = loginService.getTeacher(username);
    if (teacher != null) {
      return new User(username, teacher.getPwd(), new ArrayList<>());
    }
    Admin admin = loginService.getAdmin(username);
    if (admin != null) {
      return new User(username, admin.getPwd(), new ArrayList<>());
    } else {
