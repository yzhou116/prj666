   ...
   <div id="msg">
      ...
      <div class="wrapper">
         ...
         <ul class="bottom">
            ...
            <li class="right">
               <el-button @click="toAnswer(examData.examCode)"
                  >Start Answer</el-button
               >
            </li>
         </ul>
         <div v-if="visible">
            <el-button @click="Subscribe(examData.examCode)"
               >Subscribe</el-button
            >
         </div>
         <ul class="info">
            <li @click="dialogVisible = true">
               <a href="javascript:;"
                  ><i class="iconfont icon-info"></i>Student need to know</a
               >
            </li>
         </ul>
      </div>
            ...
            <el-collapse-item class="header" name="0">
               <template slot="title" class="stitle">
                  <div class="title">
                     ...
                     <span class="time"
                        >{{ examData.totalScore }}Score /
                        {{ examData.totalTime }}Mins</span
                     >
                     <el-button type="primary" size="small"
                        >Clck to see details</el-button
                     >
                  </div>
               </template>
               <el-collapse class="inner">
                  <el-collapse-item>
                     <template slot="title" name="1">
                        <div class="titlei">
                           MultiQuestion (Total{{ topicCount[0] }}Question
                           Total{{ score[0] }}Score)
                        </div>
                     </template>
                     <div class="contenti">
                  ...
                  <el-collapse-item>
                     <template slot="title" name="2">
                        <div class="titlei">
                           Fill Blank (Total{{ topicCount[1] }}Question Total{{
                              score[1]
                           }}Score)
                        </div>
                     </template>
                  ...
                  <el-collapse-item>
                     <template slot="title" name="3">
                        <div class="titlei">
                           True Or False (Total{{ topicCount[2] }}Question
                           Total{{ score[2] }}Score)
                        </div>
                     </template>
                     <div class="contenti">
                        ...
                        >
                           <li>
                              ...
                              {{ list.question }} {{ list.score }}Score
                           </li>
                        </ul>
                     </div>
                  </el-collapse-item>
   ...
   methods: {
      ...
      init() {
         ...
         this.$axios(`http://localhost:8080/exam/${examCode}/${username}`, {
            ...
            ///api/paper/${paperId}
            this.$axios(`http://localhost:8080/paper/${paperId}`, {
               headers,
            }).then((res) => {
               //通过paperId获取试题题目信息
               this.topic = { ...res.data };
      ...
      Subscribe(examCode) {
         ...
         this.$axios(
            `http://localhost:8080/updateExamstudent/${username}/${examCode}`,
            { headers }
         ).then((res) => {
            //通过paperId获取试题题目信息
