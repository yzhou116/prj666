  ...
  methods: {
    ...
    //  this.visible = this.$route.query.isSub 
     ...
     // console.log("the value from previous page " +  this.isSuber )
      ...
      var username = this.$route.query.useremail;
      debugger;
    ...
    toAnswer(id) {
      var examId  = this.$route.query.surveytCode;
      var useremail = this.$route.query.username;
      debugger
      this.$router.push({path:"/anonymousanswer",query:{examCode: examId, theuseremaill : useremail}})
    },
    /* Subscribe(examCode){
      var username = this.$cookies.get("cname")
      let tokenStr = this.$session.get('jwt')
