            ...
            >
               ...
               <div class="info">
                  ...
                  ><span v-if="item.totalTime != null"
                     >Limit{{ item.totalTime }}mins</span
                  >
                  <i class="iconfont icon-fenshu"></i
                  ><span>Total{{ item.totalScore }}Score</span>
               </div>
            </li>
         </ul>
...
export default {
   ...
   methods: {
      ...
      getExamInfo() {
         ...
         this.$axios(
            `http://localhost:8080/userexams/${this.pagination.current}/${this.pagination.size}/${username}`,
            { headers }
         )
            .then((res) => {
      ...
      toExamMsg(examCode) {
         this.$router.push({
            path: "/examMsg",
            query: { examCode: examCode, isSub: true },
         });
